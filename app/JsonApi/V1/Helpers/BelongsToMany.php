...
class BelongsToMany extends ToMany implements FillableToMany {
     ...
     * @param EloquentCollection $relatedModels
    ...
    private function formatAttachRecords(
        ...
        EloquentCollection $relatedModels,
        array $identifiers
    ): array {
        ...
        foreach ($pivotAttributes as $key => $keyOrDefaultValue) {
            if (is_int($key)) {
                ...
                if ($newValue = Arr::get($metaPivot, $keyOrDefaultValue)) {
                    $attributes[$this->fieldsColumns[$keyOrDefaultValue]
                        ?? Str::underscore($keyOrDefaultValue)] = $newValue;
                }
            } else {
